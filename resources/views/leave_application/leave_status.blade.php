...
<script>
    ...
    function initializeDataTable() {
        table = $('#leave_table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('leave.status') }}", // Backend URL
                data: function (d) {
                    // No need to pass status parameter since we're not filtering by status
                    // Just send any other parameters you might need (like pagination, etc.)
                }
            },
            columns: [
                ...
                },
                {
                    ...
                    render: function (data) {
                        ...
                        return !isNaN(balance) ? (balance >= 2 ? balance + ' Days' : balance + ' Day') : 'N/A';
                    },
                    orderable: false,
                    searchable: false
                },
            ...
            order: [[0, 'desc']] // Default order by the index column
        });
    }
