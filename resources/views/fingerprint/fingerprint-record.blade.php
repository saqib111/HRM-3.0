...
<script>
  $(document).ready(function () {
    const table = $('#fingerprint_table').DataTable({
      processing: true,
      serverSide: true,
      ajax: {
        ...
        type: 'GET',
        data: function (d) {
          const userId = $('#employee_name').val();  // Get user_id from hidden input
          ...
          if (userId) {
            ...
            return false;  // Prevent DataTable request if no user_id is selected
          }
        }
      },
      columns: [
        ...
        },
        {
          ...
          render: function (data, type, row) {
            return `
            <button class="btn btn-primary" onclick="editFingerprint(${row.id})"><i class="fa fa-edit"></i></button>
            <button class="btn btn-danger" onclick="showDeleteConfirmationModal(${row.id})"><i class="fa fa-trash"></i></button>
          `;
          },
          orderable: false,
          searchable: false
    ...
    $('.btn_employee').on('click', function (e) {
      ...
      if (!userId) {
        ...
        table.ajax.reload();  // Reload the table data based on the selected user and date range
      }
    });
    ...
    $('#edit_fingerprint-form').on('submit', function (e) {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data.success) {
            hideLoader();
            const row = table.row($('#edit_fingerprint-form').data('row'));
