...
class ImportScheduleLog
{
  ...
  {
    $basePath = base_path('laravel_logs/ImportSchedule');
    if (!File::exists($basePath)) {
      File::makeDirectory($basePath, 0755, true);
    }

    // Define the path of the log file
    $logPath = $basePath . '/import_schedule_data.txt'; // Directly creating update_employees.txt
    ...
    $logger = new Logger('import_schedule_log');
