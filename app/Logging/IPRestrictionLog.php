...
class IPRestrictionLog
{
  ...
  {
    $basePath = base_path('laravel_logs/IPRestriction');
    if (!File::exists($basePath)) {
      File::makeDirectory($basePath, 0755, true);
    }

    // Define the path of the log file
    $logPath = $basePath . '/ip_restriction_data.txt'; // Directly creating update_employees.txt
    ...
    $logger = new Logger('ip_restriction_log');
