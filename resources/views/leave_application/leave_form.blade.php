...
@section('content')
    ...
    <style>
            ...
            /* Style the scrollbar only for the modal content */
            #unlock_leaves_modal .modal-dialog.modal-dialog-scrollable .modal-body {
                /* Enable scrollbars for the modal body */
                overflow-y: auto;
                max-height: 75vh;
                /* Adjust the max-height as per your requirement */
            }

            /* Webkit scrollbar styles for the modal */
            #unlock_leaves_modal .modal-dialog.modal-dialog-scrollable .modal-body::-webkit-scrollbar {
                width: 8px;
                /* Set the width of the scrollbar */
            }

            #unlock_leaves_modal .modal-dialog.modal-dialog-scrollable .modal-body::-webkit-scrollbar-track {
                background: #f1f1f1;
            ...
            #unlock_leaves_modal .modal-dialog.modal-dialog-scrollable .modal-body::-webkit-scrollbar-thumb {
                background-color: rgba(223, 213, 213, 0.18);
            ...
            #unlock_leaves_modal .modal-dialog.modal-dialog-scrollable .modal-body::-webkit-scrollbar-thumb:hover {
                background-color: rgba(163, 198, 236, 0.27);
        ...
        <div class="page-header">
            <div class="row align-items-center">
                ...
                <div class="col-md-4 float-end ms-auto">
                    <div class="d-flex title-head">
                        <button class="btn btn-primary" id="unlock_leave_btn"><i class="fe fe-unlock me-2"></i>Unlock Leave
                            Categories</button>
                    </div>
                </div>
        ...
        <!-- UNLOCK LEAVES CATEGORIES MODAL STARTS -->
        <div class="modal fade" id="unlock_leaves_modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
            aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header d-flex justify-content-between">
                        <h1 class="modal-title fs-5" id="staticBackdropLabel">Request To Unlock Other Leaves</h1>
                        <button type="button" class="closed_btn" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="request_note p-2 rounded mb-3 mx-5" style="background:#f8d7da;">
                            <p><strong class="me-2">Note:</strong>
                                Please be advised that these leave types are considered special or restricted leaves,
                                which are not generally available to all employees. Therefore, to ensure that your
                                request is processed efficiently, we kindly ask you to attach the relevant supporting
                                documents when submitting your application. These documents should directly correspond
                                to the reason for your leave request.
                            <ul>
                                <li><strong>Maternity Leave:</strong> Medical certificates or proof of pregnancy</li>
                                <li><strong>Paternity Leave:</strong> Proof of partner’s delivery or medical documents</li>
                                <li><strong>Hospitalization Leave:</strong> Hospital admission or discharge records</li>
                                <li><strong>Compassionate Leave:</strong> Documents relating to the loss or illness of a close
                                    family member</li>
                                <li><strong>Unpaid Leave:</strong> Justification or reason for taking unpaid leave</li>
                            </ul>
                            </p>
                        </div>
                        <div class="card-body mx-5">
                            <form id="unlock_leave_categories_form" method="POST" enctype=multipart/form-data>
                                @csrf
                                <div class="input-block mb-3 row">
                                    <div class="col-md-12" style="padding-left:0px;">
                                        <label class="col-form-label" for="request_leave_title">Title</label>
                                        <input type="text" name="request_leave_title" id="request_leave_title"
                                            class="form-control leave-title" />
                                        <input type="hidden" value="{{$formattedLeaveBalance}}" name="balance_of_annual_leave"
                                            id="balance_of_annual_leave">
                                        <div class="text-danger" id="request_leave_title_error"></div>
                                    </div>
                                </div>
                                <div class="input-block mb-3 row">
                                    <div class="col-md-12" style="padding-left:0px;">
                                        <label class="col-form-label" for="request_leave_description">Description</label>
                                        <textarea rows="5" cols="5" name="request_leave_description"
                                            id="request_leave_description" class="form-control leave-description"
                                            placeholder="Enter text here"></textarea>
                                        <div class="text-danger" id="request_leave_description_error"></div>
                                    </div>
                                </div>
                                <div class="input-block mb-3 row">
                                    <div class="col-md-4" style="padding-left:0px;">
                                        <label class="col-form-label" for="request_leave_category">Leave Category</label>
                                        <select class="form-select mb-3" name="request_leave_category[]"
                                            id="request_leave_category">
                                            <option selected>Select Leave Category</option>
                                            <option value="4">Unpaid Leave</option>
                                            <option value="3">Marriage Leave</option>
                                            <option value="5">Hospitalization Leave</option>
                                            <option value="6">Compassionate Leave</option>
                                            <option value="7">Maternity Leave</option>
                                            <option value="8">Paternity Leave</option>
                                        </select>
                                        <div class="text-danger" id="request_leave_category_error"></div>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="col-form-label" for="request_leave_from">From</label>
                                        <input type="date" class="form-control leave-date" name="request_leave_from[]"
                                            id="request_leave_from" />
                                        <div class="text-danger" id="request_leave_from_error"></div>
                                    </div>
                                    <div class="col-md-4">
                                        <label class="col-form-label" for="request_leave_to">To</label>
                                        <input type="date" class="form-control leave-date" name="request_leave_to[]"
                                            id="request_leave_to" />
                                        <div class="text-danger" id="request_leave_to_error"></div>
                                    </div>
                                </div>
                                <div class="image-card">
                                    <div class="top">
                                        <p>Upload relevant documents images</p>
                                        <button type="button" hidden>Upload</button>
                                    </div>
                                    <div class="drag-area">
                                        <span class="visible">
                                            Drag & drop image here or
                                            <span class="image-select" role="button">Browse</span>
                                        </span>
            ...
            <!-- UNLOCK LEAVES CATEGORIES MODAL ENDS -->
@endsection

    @section('script-z')
        <script src="{{ asset('assets/js/request-leaves.js') }}"></script>
        <script>
                ...
                function updateLeaveCategories(allowedUL) {
                    ...
                    if (storedLang == "vi") {
                        if (confirmation_status_value) {
                            leaveCategoryHtml += `<option value="1">Nghỉ phép năm (AL)</option>`;
                            leaveCategoryHtml += `<option value="2">Nghỉ sinh nhật</option>`;
                            leaveCategoryHtml += `<option value="3">Nghỉ kết hôn</option>`;
                        }
                        if (allowedUL.includes(4)) {
                            leaveCategoryHtml += `<option value="4">Nghĩ không lương</option>`;
                        }
                    } else {
                        if (confirmation_status_value) {
        ...
        <!-- UNLOCK LEAVE REQUEST -->
        <script>
            $(document).ready(function () {
                $("#unlock_leave_btn").click(function (e) {
                    e.preventDefault();
                    $("#unlock_leaves_modal").modal("show");
                });

                $("#apply_btn").click(function (e) {
                    e.preventDefault();
                    var title = $("#request_leave_title").val();
                    var description = $("#request_leave_description").val();
                    var category = $("#request_leave_category").val();
                    var start_date = $("#request_leave_from").val();
                    var end_date = $("#request_leave_to").val();
                    var files = $(".file")[0].files;
                    var balance_of_annual_leave = $("#balance_of_annual_leave").val();
                    var images = [];
                    var valid = true;

                    // Validate fields
                    if (title === "") {
                        $("#request_leave_title_error").html("Title is required");
                        $("#request_leave_title").css("border", "1px solid red");
                        valid = false;
                    } else {
                        $("#request_leave_title_error").html("");
                        $("#request_leave_title").css("border", "");
                    }

                    if (description === "") {
                        $("#request_leave_description_error").html("Description is required");
                        $("#request_leave_description").css("border", "1px solid red");
                        valid = false;
                    } else {
                        $("#request_leave_description_error").html("");
                        $("#request_leave_description").css("border", "");
                    }

                    if (category === "Select Leave Category") {
                        $("#request_leave_category_error").html("Category is required");
                        $("#request_leave_category").css("border", "1px solid red");
                        valid = false;
                    } else {
                        $("#request_leave_category_error").html("");
                        $("#request_leave_category").css("border", "");
                    }

                    if (start_date === "") {
                        $("#request_leave_from_error").html("Start date is required");
                        $("#request_leave_from").css("border", "1px solid red");
                        valid = false;
                    } else {
                        $("#request_leave_from_error").html("");
                        $("#request_leave_from").css("border", "");
                    }

                    if (end_date === "") {
                        $("#request_leave_to_error").html("End date is required");
                        $("#request_leave_to").css("border", "1px solid red");
                        valid = false;
                    } else {
                        $("#request_leave_to_error").html("");
                        $("#request_leave_to").css("border", "");
                    }
                    ...
                    if (valid) {
                        showLoader();
                        // Use FormData to handle file uploads along with form data
                        var formData = new FormData($('#unlock_leave_categories_form')[0]);
                        if (images.length > 0) {
                        ...
                        $.ajax({
                            url: "{{route('unlock.leaveApplication')}}",
                            type: "POST",
                            data: formData,
                            ...
                            success: function (result) {
                                ...
                                $("#request_leave_title").val("");
                                $("#request_leave_description").val("");
                                $("#request_leave_category").val("");
                                $("#request_leave_from").val("");
                                $("#request_leave_to").val("");
                                $(".file").val("");

                                // Hide modal and show success toast
                                $("#unlock_leaves_modal").modal("hide");
                                hideLoader();
                                createToast('info', 'fa-solid fa-circle-check', 'Success', result.message);
                            },
                            error: function (error) {
                                ...
                                createToast('error', 'fa-solid fa-circle-exclamation', 'Error', message);
                            }
                        });
                    }
                });
